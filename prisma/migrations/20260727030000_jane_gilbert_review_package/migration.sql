...
VALUES
('obs_src_jg_appointment','https://www.miamidade.gov/global/release.page?Mduid_release=rel1671058592011897','Mayor Daniella Levine Cava and Miami Foundation launch first-ever Extreme Heat Action Plan','Miami-Dade County','government_release','2022-12-14T00:00:00Z','2026-08-29T00:00:00Z',true,'public',CURRENT_TIMESTAMP),
...
('obs_src_jg_plan','https://www.miamidade.gov/resources/environment/documents/2022-heat-action-plan.pdf','Extreme Heat Action Plan','Miami-Dade County','government_plan','2022-12-14T00:00:00Z','2026-07-26T00:00:00Z',true,'public',CURRENT_TIMESTAMP),
...
VALUES
('obs_claim_jg_c01',(SELECT "id" FROM "ObservatoryCase" WHERE "slug"='jane-gilbert'),'county_appointment','Miami-Dade County appointed Jane Gilbert as the world’s first Chief Heat Officer in 2021.','role_formation','observed','unreviewed',NULL,'Miami-Dade appointed Jane Gilbert as its inaugural Chief Heat Officer in 2021.','Preserve county scope. The exact announcement date, any initial interim status and the initial hosting arrangement are not established by a currently retrievable primary record.','draft',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP),
...
VALUES
('obs_rel_jg_county',(SELECT "id" FROM "ObservatoryCase" WHERE "slug"='jane-gilbert'),'government','Miami-Dade County','chief_heat_officer','2021-01-01T00:00:00Z','2025-06-30T00:00:00Z','unreviewed','draft','obs_src_jg_appointment',NULL,'Start recorded to the year; end date approximate pending personnel confirmation',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP),
('obs_rel_jg_foundation',(SELECT "id" FROM "ObservatoryCase" WHERE "slug"='jane-gilbert'),'organization','The Miami Foundation','launch_partner',NULL,NULL,'unreviewed','draft','obs_src_jg_appointment',NULL,'Named by the county as a partner in launching the Extreme Heat Action Plan; any role-hosting arrangement is not established by a currently retrievable record',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP),
...
VALUES
('obs_event_jg_appointment',(SELECT "id" FROM "ObservatoryCase" WHERE "slug"='jane-gilbert'),'appointment','Gilbert appointed Chief Heat Officer','County-level appointment; the county describes it as the world’s first post of its kind.','2021-01-01T00:00:00Z','year','unreviewed','draft','obs_src_jg_appointment',NULL,'Year established; exact announcement date not retrievable from a live primary record',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP),
