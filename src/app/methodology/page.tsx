...
export default function MethodologyPage() {
  return <main className="method-page method-2">
    ...
    <p className="method-hook">Then ask what the person actually built, what can leave with them, what they control, and what still depends on someone else.</p>
    ...
    <section className="method-limits"><div><p className="eyebrow">Interpret with care</p><h2>What the project cannot claim—yet.</h2></div><ol>{limits.map((limit,i)=><li key={limit}><span>{String(i+1).padStart(2,"0")}</span><p>{limit}</p></li>)}</ol></section>

    <section className="method-next"><p className="eyebrow">How the method becomes trustworthy</p><h2>Ask. Listen. Test. Revise. Repeat.</h2><p>Next come expert review, interviews about how people understand each question, missing-data and item analysis, reliability and factor testing, comparison with established measures, fairness checks, and longitudinal research. Material changes receive new version numbers.</p><a href="/methodology/candidates">Enter the Question Lab →</a></section>
    <div className="actions"><a href="/assess" className="primary-link">Choose the question about your work</a><a href="/observatory" className="text-link">See the framework meet real careers →</a></div>
  </main>;
}