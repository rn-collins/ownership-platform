...
export default async function ObservatoryProfile({ params }: { params: { slug: string } }) {
  ...
  return <main className={styles.page}>
    <Link href="/observatory" className={styles.back}>← Back to all 41 careers</Link>
    <header className={styles.hero}>
      ...
      <div className={styles.meta}><span>{node.domain}</span><span>{lens} case</span>{node.created && <span>role shaped around a person</span>}</div>
    </header>

    <section className={styles.question} aria-labelledby="central-question"><span>The question this career opens</span><h2 id="central-question">{node.question ?? "What does this career make possible—and what makes it fragile?"}</h2><p>This is an investigation, not a verdict about the person.</p></section>

    <div className={styles.storyGrid}>
      <section className={styles.section}><h2>Why this case matters</h2><p>{node.name} gives us a concrete way to examine <strong>{(node.tension ?? "ownership and dependence").toLowerCase()}</strong> in {node.domain.toLowerCase()}. The public role tells us where to begin. It does not tell us everything the person owns, controls, can carry elsewhere, or depends on to continue.</p><p>The useful move is to separate the visible career from its underlying structure: audience, employer, collaborators, intellectual property, distribution, revenue, reputation, and decision-making power.</p></section>
      <aside className={styles.aside}><h3>What this page can—and cannot—say</h3><p>{reviewed ? `The claims displayed below have passed the current public-evidence review${reviewedAt ? ` as of ${date(reviewedAt)}` : ""}.` : "The role description has been checked and narrowed, but the full evidence review is still in progress."}</p><p>Public sources rarely reveal complete contracts, private economics, informal power, or what would happen if a key relationship ended. We do not fill those gaps with guesses.</p></aside>
    </div>

    {claims.length > 0 ? <section className={styles.evidence} aria-labelledby="record-heading"><p className={styles.kicker}>The public record</p><h2 id="record-heading">What we can responsibly say</h2><p className={styles.evidenceIntro}>These are bounded claims supported by the sources attached to them. Open the evidence only when you want to inspect how a statement was established or qualified.</p>{claims.map((claim) => <article className={styles.claim} key={claim.id}><span className={styles.label}>{claim.claimType.replaceAll("_", " ")} · {claim.verificationStatus.replaceAll("_", " ")}</span><h3>{claim.permissibleLanguage || claim.statement}</h3>{claim.contradictionNote && <p><strong>Important qualification:</strong> {claim.contradictionNote}</p>}<details><summary>Inspect the evidence ({claim.evidence.length})</summary><div className={styles.sources}>{claim.evidence.length ? <ol>{claim.evidence.map((item) => <li key={item.id}><a href={item.source.url} target="_blank" rel="noreferrer">{item.source.title}</a>{item.source.publisher ? ` — ${item.source.publisher}` : ""}{item.source.publishedAt ? ` (${date(item.source.publishedAt)})` : ""}{item.source.primarySource ? " · Primary source" : ""}{item.exactPassage && <blockquote>{item.exactPassage}</blockquote>}{item.locator && <p>Location: {item.locator}</p>}</li>)}</ol> : <p>No public citation is attached yet.</p>}</div></details></article>)}</section> : <section className={styles.unknown}><h2>What remains open</h2><p>The full public-evidence review for this case is not finished. We still need dated sources that establish the important relationships, turning points, ownership claims, dependencies, and contrary evidence. Until then, an unanswered question remains unanswered; missing information is not proof that something does not exist.</p></section>}

    <section className={styles.next}><p className={styles.kicker}>Keep following the pattern</p><h2>Compare this career with another arrangement.</h2><div className={styles.links}>{related.map((person) => <Link href={`/observatory/${nodeSlug(person.name)}`} key={person.name}>{person.name} →</Link>)}</div></section>
  </main>;
}